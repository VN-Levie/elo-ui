...
const sortedMatchHistory = computed(() => {
    ...
    // Sort descending by numeric part of matchId (e.g. match_16053 -> 16053)
    return [...player.value.matchHistory].sort((a, b) => {
        const getNum = id => Number(String(id).split('_').pop()) || 0;
        return getNum(b.matchId) - getNum(a.matchId);
    });
});
